...
func (r *ResourceProvisioner) Apply(
	...
	provisioner.PreventSudo = true
	// ensure that this is a linux machine
	if s.Ephemeral.ConnInfo["type"] != "ssh" {
		return fmt.Errorf("Unsupported connection type: %s. This provisioner currently only supports linux",
			s.Ephemeral.ConnInfo["type"])
	}
	...
	if err := provisioner.Provision(o, comm); err != nil {
		return err
	}
	return nil
}
